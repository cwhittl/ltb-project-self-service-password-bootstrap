 ...
 * @param lang language
 */
function selfservice_recaptcha_get_conf($recaptcha_theme, $lang) {
    if($recaptcha_theme == "custom"){
        $custom = ', custom_theme_widget: \'recaptcha_widget\'';
    }else{
        $custom = "";
    }
    printf('
    ...
    </script>', $lang, $recaptcha_theme,$custom);
}
function selfservice_recaptha_get_html($recaptcha_publickey, $error = null, $use_ssl = false, $rc_theme = "clean",$lang){
    global $recaptcha_theme_template;
    $rc_template = "";
    if($rc_theme == "custom" && !empty($recaptcha_theme_template)){
        $rc_template = $recaptcha_theme_template;
    }else if($rc_theme == "custom" && empty($recaptcha_theme_template)){
        $rc_theme = "clean";
        $rc_template = "";
    }
    $rc_conf = selfservice_recaptcha_get_conf($rc_theme, $lang);
    $rc = recaptcha_get_html($recaptcha_publickey, $error, $use_ssl);
    return $rc_conf . $rc_template . $rc;
}
